...
test('отрицательный тест: сквозная перенумерация не порождает ложных потерь', () => {
  ...
  // После включения 20 обязанностей прежней роли проверяем точные источники,
  // вручную разобранные в docs/loss-review.md, вместо общего порога количества.
  assert.deepEqual(lost.flatMap((f) => f.evidenceBefore.map((e) => e.number)).sort(),
    ['5.3.3', '5.3.4', '5.3.11', '5.4.10', '5.6.2', '5.6.3', '5.7.2'].sort());
