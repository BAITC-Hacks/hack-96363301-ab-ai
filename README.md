...
HackAlem AI 2026 · Трек 12, спец-трек **Astana Innovations / Казактелеком** · Команда **Ab.Ai** (1 участник)

> **Проверяющему: API-ключ не нужен.** Весь сценарий проходится на записанных ответах модели.
> Две команды — и вы увидите результат: `cd server && npm install && npm start`, затем `npm run verify`.
...
| 11 | Демо-режим без API-ключа на записанных фикстурах |
...
fixtures/
  ...
  llm/                     записанные ответы модели для демо-режима
...
npm start
```
С ключом объяснения генерируются моделью и дописываются в `fixtures/llm/`. Без ключа берутся оттуда же.
</details>

## 8. Как проверить решение

**Первое и главное: приложение проходит весь сценарий без API-ключа.** Детерминированные шаги — разбор документов, сопоставление пунктов, валидация ссылок — выполняются по-настоящему; ответы модели воспроизводятся из `fixtures/llm/`. В интерфейсе это видно плашкой демо-режима и колонкой «источник» в панели трассировки.
...
- **Внешние сервисы** — только OpenAI API (`gpt-4.1-mini`) для объяснений и заключения. Опционален: без ключа работают фикстуры.
...
- **Демо-режим и свои документы.** Без API-ключа объяснения модели доступны только для выданного комплекта: для чужих файлов записанных фикстур нет. Детерминированная часть — состав подразделений, потери, переносы, дублирование, конфликты, заключение — работает и на чужих документах без ключа.
