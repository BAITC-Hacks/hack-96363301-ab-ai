...
server/
  ...
  fixtures/llm/            один сохранённый ответ модели для синтетического учебного примера
...
Роль модели — **верификация уже найденного**: она сверяет сопоставление и его источники, а её ответы записываются в `server/fixtures/llm/` для последующего воспроизведения. **В поставке один записанный ответ для синтетического учебного примера.** Живой анализ `/api/analyze/example` получил ответ API: приняты 2 из 2 соответствий, возвращены 3 ссылки, отклонённых ссылок — 0. Для тех же файлов, имён и модели проверено воспроизведение сохранённого ответа без ключа (`source: fixture`). Контрольный комплект организатора вживую через модель не проверялся; качество смысловых выводов этим прогоном не оценено. Подробности — в [docs/live-api-check.md](docs/live-api-check.md). Колонка «источник» в трассировке различает живой API, сохранённый ответ и отсутствие ответа.

Принятые метки и ссылки не означают принятия свободного текста модели. В сохранённом ответе о кандидате на утрату сказано «была удалена». `applyModelExplanations` не публикует этот исходный `rationale`: пользователь получает локальное пояснение о ненайденном соответствии и необходимости проверки возможной утраты или переформулировки.
...
- **Проверка API** — выполнены короткая проверка подключения и живой анализ синтетического учебного комплекта на двух соответствиях. Сохранён один ответ модели; его повторное применение без ключа проверено. Документы организатора вживую через API не проверялись, качество смыслового анализа не оценивалось. При настройке ключа приложение отправляет выбранные фрагменты документов в API для верификации. Результаты и ограничения: [docs/live-api-check.md](docs/live-api-check.md).
...
- **Модельная проверка ограничена одним учебным прогоном.** В `server/fixtures/llm/` сохранён один ответ для двух соответствий синтетического примера. Без ключа он воспроизводится только для совпадающего запроса и модели; при отсутствии подходящего ответа шаг недоступен. Живой прогон документов организатора и оценка качества смысловых выводов не выполнены. Детерминированное ядро формирует состав подразделений, кандидатов на утрату, переносы, пересечения, потенциальные конфликты и заключение без ключа на документах поддерживаемой структуры.
...
- **Модель приложения:** по умолчанию OpenAI `gpt-4.1-mini` через опциональный API (`OPENAI_MODEL` позволяет сменить модель). Короткая живая проверка подключения вернула идентификатор `gpt-4.1-mini-2025-04-14`; отдельно проверены два соответствия синтетического учебного примера. Своих и дообученных моделей нет.
