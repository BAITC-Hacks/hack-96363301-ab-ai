...
| Функции | 14 изменений состава владельцев (включая частичные), 4 кандидата на утрату, 23 добавленных закрепления, 3 переформулировки, 44 без изменений |
...
Частичное изменение состава владельцев теперь тоже отображается как передача/перераспределение. Если одинаковый текст появился в отдельном пункте другого подразделения, новое закрепление не скрывается. Короткие вводные к перечислениям не считаются отдельными функциями. Содержательная проверка пяти прежних кандидатов на утрату — в [docs/loss-review.md](docs/loss-review.md): один оказался вводным пунктом, четыре требуют проверки владельцами процессов.
...
В `docs/screenshots/` файлы `01`–`06` — исторические снимки до исправлений; `07-responsibility-map.png`, `08-decision-workbench.png`, `09-organizer-map.png` показывают актуальную карту и план. Сквозная браузерная проверка: при запущенном сервере и отдельном Chrome с `--remote-debugging-port=9333` выполнить `cd server && node scripts/browser-smoke.js`. Флаг `--screenshots` обновляет снимки. Проверка использует отдельный профиль браузера; она очищает только ключи `orgdiff.plan.*` этого профиля.

API плана: `POST /api/plan` и `POST /api/plan/export`, тело `{ reportId, decisions }`. `reportId` выдаёт сервер анализа. Клиент не присылает свой отчёт; сервер проверяет находку, допустимое действие, существующего владельца, непустое обоснование и принадлежность источников. Для подтверждения передачи нужны обе редакции. Экспорт — XLSX, четыре листа, ссылки из плана на цитаты. Сервер проверяет структуру и источники решения, но не подменяет содержательное согласование ответственным сотрудником.
...
- **Рекомендации** формируются по типу находки. В плане сотрудник может предложить перераспределение ответственности, обосновать его и получить проект формулировки. Автоматического оптимизатора оргструктуры нет.
