...
export function EvidenceItem({ ev }: { ev: Evidence }) {
  return (
    <li className="min-w-0 border-l-2 border-slate-300 bg-slate-50 py-1.5 pr-2 pl-3">
      <div className="mb-1 flex flex-wrap items-center gap-2">
        ...
        <span className="max-w-full break-all font-mono text-[11px] text-slate-500">{ev.ref}</span>
      </div>
      ...
      <blockquote className="break-words text-[13px] text-slate-800 italic">«{ev.text}»</blockquote>
    </li>
  )
}
...
export function RefChips({ evidence }: { evidence: Evidence[] }) {
  return (
    <span className="ml-1 inline-flex max-w-full flex-wrap gap-1 align-middle">
      {evidence.map((ev, i) => (
        <span
          key={`${ev.ref}-${i}`}
          title={`${ev.fileName || ev.fileId ? `${ev.fileName || ev.fileId}\n` : ''}${ev.text}`}
          className="max-w-full break-all rounded-sm border border-slate-300 bg-slate-100 px-1 text-[11px] text-slate-700"
        >
          {(ev.fileName || ev.fileId) && <span className="font-medium">{ev.fileName || ev.fileId} · </span>}
